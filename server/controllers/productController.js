...
export const postProductReview = catchAsyncErrors(async (req, res, next) => {
  ...
  // 🔥 Check: User purchased AND order delivered?
  ...
  if (rows.length === 0) {
    return res.status(403).json({
      success: false,
      message: "You can only review this product after delivery.",
    });
  }
  ...
  // Update avg rating
  const avgRatingQuery = await database.query(
  ...
  const updatedProduct = await database.query(
    ...
    [avgRatingQuery.rows[0].avg_rating, productId]
  );
...
export const deleteReview = catchAsyncErrors(async (req, res, next) => {
  ...
  if (rows.length === 0) {
    return res.status(403).json({
      success: false,
      message: "You can delete the review only after the product is delivered.",
    });
  }
